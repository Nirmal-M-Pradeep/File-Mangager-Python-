...
# Disable icon size options initially
    ...
    def open_recycle_bin(self):
        """Navigate to the system's Trash/Recycle Bin folder."""
        import sys
        import os
    
        if sys.platform == "win32":
            # Windows Recycle Bin is not a real folder; show a message
            QMessageBox.information(
                self,
                "Recycle Bin",
                "Opening the Recycle Bin is not supported on Windows in this file manager."
            )
            return
        ...
        if os.path.exists(trash_path):
            self.navigate_to_path(trash_path)
        else:
            QMessageBox.warning(
                self,
                "Trash Not Found",
                f"Trash folder not found at:\n{trash_path}"
            )
        ...
        if msg_box.exec() == QMessageBox.StandardButton.Yes:
            ...
            for file_info in selected_files:
                try:
                    ...
                    self.handle_delete_error(str(e), file_info['name'])
                    on_delete_complete()
